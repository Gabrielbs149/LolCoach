...
/**
  ...
  async function montarVivo() {
    ...
    // último perigo falado (fora kills/timers): a fala de morte diz "tinha aviso" se você morreu até 12 s depois
    const ultimoPerigoT = partidaVivo.falas.findLast?.((f) => (f.prioridade ?? 1) >= 3 && f.modulo !== 'kills' && f.modulo !== 'timers')?.t ?? null;
    for (const f of [...falasNovas({ estado, rastreio, objetivos: objs, conselhos, extras: partidaVivo.extras, olho: !!partidaVivo.olho?.calib && Date.now() - partidaVivo.olho.recebidoEm < 5000, ultimoPerigoT }, partidaVivo.memFalas), ...falasDeFlash(estado.tempo)]) {
